...
|   Based on our EDA, it seems likely that a log transformation of both gross revenue and budget may be helpful to produce a better-fitting linear model. The dataset was divided into two parts: 30% for model building and 70% for model validation. The first set portion of the dataset was used to produce multiple linear models and evaluate them using coefficient tests in R. Stargazer was used to compared the models to determine which model best aligns with the data and our causal theory. Armed with this chosen model, its predictions ever evaluated against the test data to check how the model stood up when applied to the second set of data that we saved for validation. Finally, the model was applied to the specific case outlined in the overview section above to predict the revenue for both a PG-13 and R MPAA rating case.
...
#Model 1: Rating Only
# Base Case - R Movies
model_1 = lm(log(gross) ~ PG13, data = train)

#Model 2: Rating and Quality
# Base Case - R Movies with rating of 0
model_2 = lm(log(gross) ~ PG13 + score, data = train)

#Model 3: Rating and Quality with interaction terms
model_3 = lm(log(gross) ~ PG13 + score + PG13*score, data = train)

#Model 4: Rating and Quality with budget
model_4 = lm(log(gross) ~ PG13 + score + log(budget), data = train)

#Model 5: Rating and Quality with budget and interaction terms
model_5 = lm(log(gross) ~ PG13 + score + PG13 * score + log(budget) + log(budget) * PG13, data = train)
...
# test
model_6 = lm(score ~ PG13, data = train)
summary(model_6)
```
...
```{r model1 coeff interp}
m1_int = (exp(16.770))
m1Rto_PG13 = (exp(1.044)-1)*100

m1_int
m1Rto_PG13
```
> 184% percent change in gross if rating changes from R to PG13?? is this right?
> 19.2 million gross for R movies


|   The second model (2) introduced the effect of a film's IMDb score on the gross revenue alongside the basic coefficients. $\beta_2$ indicates the estimated gain in revenue per increase in IMDb score.

(2) $$ ln(Gross) = \beta_0 + \beta_1 * PG13 + \beta_2 * Score $$

```{r model2 coeff interp}
m2_int = (exp(13.659))
m2Rto_PG13 = (exp(1.142)-1)*100
m2_score =(exp(0.480)-1)*100
...
> 213% percent change in gross if rating changes from R to PG13 while holding score constant?? is this right?
> 855k gross for R movies with zero rating (this does not matter for model)
> 61.6% increase in gross if rating goes up 1 point

|   The third model (3) introduced the possible effect of the interaction of a film's IMDb score and ratings on the gross revenue alongside the basic coefficients. $\beta_3$ indicates the estimated extra gain in revenue per increase in IMDb score if the movie is rated PG-13.

(3) $$ ln(Gross) = \beta_0 + \beta_1 * PG13 + \beta_2 * Score + \beta_3 * PG13 * Score $$


```{r model3 coeff interp}

```


|   The fourth model (4) introduced the possible effect of a film's budget on the gross revenue alongside the basic coefficients for rating and score. $\beta_3$ indicates the estimated extra gain in revenue per increase in IMDb score if the movie is rated PG-13.

(4) $$ ln(Gross) = \beta_0 + \beta_1 * PG13 + \beta_2 * Score + \beta_3 * ln(Budget)$$

```{r model4 coeff interp}

```

|   The fifth model (5) introduced the possible effect of the interaction of a film's IMDb score and ratings as well as the interaction between MPAA rating and budget on the gross revenue alongside the basic coefficients. $\beta_3$ indicates the estimated extra gain in revenue per increase in IMDb score if the movie is rated PG-13. $\beta_4$ indicates the estimated extra gain in revenue per increase in IMDb score if the movie is rated PG-13. $\beta_5$ indicates the estimated extra gain in revenue per increase in budget if the movie is rated PG-13.

(5) $$ ln(Gross) = \beta_0 + \beta_1 * PG13 + \beta_2 * Score + \beta_3 * PG13 * Score  + \beta_4 * ln(Budget) + \beta_5 * PG13 * ln(Budget) $$

```{r model5 coeff interp}
...
```{r - Model validation}
library(forecast)
# Example is Model 5 but can be changed
test$predicted = predict(model_5, test)
test$residuals = log(test$gross)-test$predicted

test %>%
  ggplot(aes(x=predicted, y=(log(gross)-predicted))) +
  geom_point() +
  ggtitle("Model Residuals vs Prediction Plot") +
