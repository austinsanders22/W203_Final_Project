...
```{r - EDA: Check other numeric explanatory variables, warning=FALSE}
...
```{r - EDA: Distribution of Ratings}

df$rating_bin = factor(ifelse(df$rating == "R", "R", ifelse(df$rating == "PG-13", "PG-13", "G/PG")))

df %>%
  ggplot(aes(x=rating_bin, fill=rating_bin)) +
  geom_bar() +
...
|   There doesn't seem to be a strong correlation between the year and revenue of the movie.
